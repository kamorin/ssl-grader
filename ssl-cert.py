...
if __name__ == "__main__":
    ...
    for cert in certs:
        table.add_row([", ".join(cert.hostname), cert.subject, ", ".join(cert.altnames) + "\n\n", cert.grade, ", ".join(cert.issues) + "\n\n"])
    table.sortby = "Grade"
    print(table)
    ...
    if csv_output:
        with open(domain + ".csv", "w", newline="") as csvfile:
            ...
            for cert in certs:
                certwriter.writerow([", ".join(cert.hostname), cert.subject, ", ".join(cert.altnames), cert.grade, ", ".join(cert.issues)])
